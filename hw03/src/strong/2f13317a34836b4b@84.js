...
Inputs.color({ label: "background color", value: "#e8cedd" })
)}

function _strokeColor(Inputs){return(
Inputs.color({ label: "stroke color", value: "#272025" })
)}
...
(width, height, offsetX, offsetY, scale) => {
  ...
  const colorRange = thresholds.map(value => d3.interpolateBuPu(value / maxValue));
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["UserData.json", {url: new URL("../UserDate.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
